...
class IPAudioScreen(Screen):
    ...
    def iniMenu(self, sList):
        ...
        for elem in sList:
            if isHD():
                res.append(MultiContentEntryText(pos=(0, 0), size=(0, 0), font=0, flags=RT_HALIGN_LEFT | RT_VALIGN_CENTER | RT_WRAP, text='', border_width=3))
                res.append(MultiContentEntryText(pos=(5, 2), size=(580, 50), font=0, backcolor_sel=None, flags=RT_VALIGN_CENTER | RT_HALIGN_LEFT, text=str(elem[0])))
            else:
                res.append(MultiContentEntryText(pos=(0, 0), size=(0, 0), font=0, flags=RT_HALIGN_LEFT | RT_VALIGN_CENTER | RT_WRAP, text='', border_width=3))
                res.append(MultiContentEntryText(pos=(5, 7), size=(580, 50), font=0, backcolor_sel=None, flags=RT_VALIGN_CENTER | RT_HALIGN_LEFT, text=str(elem[0])))
            gList.append(res)
            res = []
        return gList
    ...
    def audioStart(self):
        if fileExists('/dev/dvb/adapter0/audio10') and config.plugins.IPAudio.running.value and not isMutable():
            ...
            IPAudioHandler.container.execute(cmd)
    ...
    def runCmd(self, cmd):
        if self.alsa:
            ...
            if not config.plugins.IPAudio.keepaudio.value:
                if not IPAudioHandler.container.running() and isMutable():
                    cmd = '{} Mute && {}'.format(config.plugins.IPAudio.player.value, cmd)
                else:
                    if not IPAudioHandler.container.running() and fileExists('/dev/dvb/adapter0/audio0'):
                        self.session.nav.stopService()
        ...
        if not config.plugins.IPAudio.lastplayed.value == "e2_service":
            if fileExists('/dev/dvb/adapter0/audio10') and config.plugins.IPAudio.running.value:
                ...
                if config.plugins.IPAudio.running.value and isMutable():
                    self.stopIPAudio()
                    self.container.execute('{} Unmute'.format(config.plugins.IPAudio.player.value))
                    config.plugins.IPAudio.running.value = False
                    config.plugins.IPAudio.running.save()
